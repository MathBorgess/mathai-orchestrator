...
def preamble_text(
    *,
    ...
    """Paths, never content. The handoff is data, not a command (SPEC §2.2)."""
    lines = [
...
def run_check(
    ...
    *,
    ...
    """Deterministic, zero LLM, argv without a shell. The timeout is the parent's:
    `timeout(1)` does not ship on macOS and is banned by SPEC §3.5-3. The check goes
    into its own process group like every other node, so the stop can kill it."""
    log_dir.mkdir(parents=True, exist_ok=True)
    cwd.mkdir(parents=True, exist_ok=True)
    stdout_path = log_dir / f"{node.id}.jsonl"
    stderr_path = log_dir / f"{node.id}.err"
